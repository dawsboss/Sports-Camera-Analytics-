...
# until it is copied to the homelab's MinIO; it goes nowhere else.
#
...
PATH_MAIN=${PATH_MAIN:-main}          # Milesight main stream; check the camera's RTSP page
declare -A CAMS=(
  [far-l]=192.168.50.11
  [far-r]=192.168.50.12
...
for name in "${!CAMS[@]}"; do
  ffmpeg -nostdin -loglevel warning -rtsp_transport tcp \
    -i "rtsp://${CAM_AUTH}@${CAMS[$name]}:554/${PATH_MAIN}" -map 0 -c copy \
